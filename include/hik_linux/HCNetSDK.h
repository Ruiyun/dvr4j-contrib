...
#define NET_DVR_GET_DEFAULT_VIDEO_EFFECT		6136		// 获取视频输入效果参数默认值
...
#define NET_DVR_SET_VIDEO_EFFECT				6137       //设置通道视频输入图像参数
...
typedef struct 
{
	...
	BYTE byRecordType;	//0:定时录像，1:移动侦测，2:报警录像，3:动测|报警，4:动测&报警, 5:命令触发, 6: 智能录像,10-PIR报警，11-无线报警，12-呼救报警，13-移动|报警输入|PIR|无线报警|呼救报警
	char reservedData[3];
...
typedef struct 
{
	...
	BYTE byRecordType;				/* 录象类型 0:定时录像，1:移动侦测，2:报警录像，3:动测|报警，4:动测&报警 5:命令触发, 6: 智能录像,10-PIR报警，11-无线报警，12-呼救报警，13-移动|报警输入|PIR|无线报警|呼救报警*/
	char reservedData;
...
typedef struct _NET_DVR_SCREENALARMCFG
{
	...
	BYTE	bySubException;	//输入异常时具体子异常 1- 分辨率正常改变 2-输入端口类型改变3-分辨率错误4-分辨率改变导致解码资源不足，关闭该输入源对应窗口。5-分辨率改变，导致已开窗的缩放比例不在1/8到8倍范围。
	BYTE	byRes1; 
...
NET_DVR_API BOOL __stdcall NET_DVR_CommandDevice(LONG lUserID, DWORD dwOutCommand, const void * lpInBuffer, DWORD dwInLen);
